...
import { Plus, Trash2, Printer, ArrowLeft, Check } from 'lucide-react';
...
function PrintablePO({
  ...
  const totalValue = lines.reduce(
    (s, l) => s + l.ordered_qty * (parseFloat(l.unit_cost) || 0),
    0,
  );
...
function CreatePOForm() {
  ...
  async function handleSubmit() {
    ...
    // site_id: fetch first site
    try {
      const siteRes = await apiClient<{ data: Array<{ id: string }> }>('/locations?type=bin&per_page=1');
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const siteId: string = (siteRes as any)?.data?.[0]?.site_id ?? '';
      payload.site_id = siteId;

      const result = await createPO.mutateAsync({ ...payload, site_id: siteId });
  ...
  return (
    <DeskLayout heading="Create Purchase Order" title="Create PO">
      <div style={{ maxWidth: '960px' }}>
        ...
        {/* Header fields */}
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: '16px', marginBottom: '24px', backgroundColor: 'var(--surface)', border: '1px solid var(--border)', borderRadius: '8px', padding: '20px' }}>
          <div>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr style={{ backgroundColor: 'var(--surface-sunken)' }}>
                  {['#', 'SKU Code', 'Description', 'Qty', 'Unit Cost (₹)', 'Amount', ''].map((h) => (
        ...
        <div style={{ display: 'flex', gap: '10px' }}>
          ...
          <button
            ...
            style={{ padding: '10px 24px', backgroundColor: 'var(--brand-primary)', color: '#fff', border: 'none', borderRadius: '6px', cursor: 'pointer', fontSize: '13px', fontWeight: 600, fontFamily: 'inherit', opacity: createPO.isPending ? 0.6 : 1 }}
          >
